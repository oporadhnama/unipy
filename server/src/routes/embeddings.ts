...
(await embeddingsRouter.get('/', async (_req: Request, res: Response) => {
  const db = getDb();
  ...
  for (const row of (await listEmbeddingModels())) {
    const list = (await byFamily.get(row.family)) ?? [];
    list.push(row);
  ...
  res.json({
    ...
    families: (await [...byFamily.entries()].map(async ([family, rows]) => ({
          ...
          providers: (await rows.map(async r => ({
                  ...
                  keyCount: (await keyCounts.get(r.platform)) ?? 0,
                }))),
        }))),
  });
}));
...
embeddingsRouter.put('/', async (req: Request, res: Response) => {
  ...
  if (parsed.data.defaultFamily) {
    const exists = await db.prepare('SELECT 1 FROM embedding_models WHERE family = ?').get(parsed.data.defaultFamily);
    if (!exists) {
      res.status(400).json({ error: { message: `Unknown family '${parsed.data.defaultFamily}'` } });
      return;
    }
    (await setSetting('embeddings_default_family', parsed.data.defaultFamily));
  }

  if (parsed.data.providers) {
    const update = db.prepare('UPDATE embedding_models SET priority = ?, enabled = ? WHERE id = ?');
    const apply = db.transaction(async (rows: { id: number; priority: number; enabled: boolean }[]) => {
      for (const r of rows) (await update.run(r.priority, r.enabled ? 1 : 0, r.id));
    });
    (await apply(parsed.data.providers));
  }
...
(await embeddingsRouter.get('/usage', async (_req: Request, res: Response) => {
  const db = getDb();
